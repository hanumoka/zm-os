...
>
> ★ **2026-09-03 정정: 이 문장이 가리키는 `ADR-0024+`는 작성되지 않는다.** 인덱스가 그 번호대를 **소멸**로 닫았고(Supabase·Cloud 트랙 폐기) *"결번 0024~0029는 회수하지 않는다"* 고 적었다. 위 문장을 따라가면 존재하지 않는 문서를 찾게 된다 — 보존된 결정 근거는 그 자리에서 참고 자료로만 남는다.
