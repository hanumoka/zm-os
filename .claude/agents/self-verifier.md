...
### 5. 표준 절차 준수 (2단계 검증 파이프라인)
- **architect 필수 게이트**: architect 설계 보고가 존재하는가 (SKIP 불가)
- **design-reviewer**: 설계 적합성 검증 PASS를 받았는가 (SKIP 불가)
- **1차 검증**: build-checker / code-reviewer / app-sandbox-auditor (해당 시) / constraint-checker 4명 모두 PASS 받았는가
- **2차 검증**: integration-tester / perf-monitor PASS를 받았는가
